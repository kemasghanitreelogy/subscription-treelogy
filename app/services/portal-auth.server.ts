...
export async function authorizePortalAction(
  ...
  if (!token) return Response.json({ error: "Token wajib" }, { status: 401 });

  const grant = await consumePortalToken(db, token, scope);
  if (!grant || grant.subscriptionId !== subscriptionId) {
    return Response.json({ error: "Link kedaluwarsa atau tidak sah — minta link baru" }, { status: 401 });
  }

  const { rows } = await db.query("select * from subscriptions where id = $1", [subscriptionId]);
  if (!rows.length) return Response.json({ error: "Langganan tidak ditemukan" }, { status: 404 });
